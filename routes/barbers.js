...
var mongodbUri = 'mongodb://loti.ibrahimi:Prishtina8991MLAB@ds137863.mlab.com:37863/getsprucedb';
...
router.findOne = (req, res) => {
    ...
    Barbers.find({ "_id" : req.params.id },function(err, foundBarber) {
        if (err)
            ...
            res.send(JSON.stringify(foundBarber,null,5))
    });
};
...
router.findByRegion = (req, res) => {
    ...
    Barbers.find({'region' : req.params.region}, function(err, foundBarbers) {
        if (err) {
            ...
            res.send(JSON.stringify(foundBarbers,null,5))
        }
    })
};
